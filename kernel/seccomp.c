...
/*
 * Stub seccomp verifier ops - needed for BPF_PROG_TYPE_SECCOMP registration
 * in the BPF program type system. Actual seccomp filters use classic BPF
 * converted via bpf_prog_create_from_user().
 */
...
static struct seccomp_filter *seccomp_prepare_filter(struct sock_fprog *fprog)
{
	...
	int ret;
	const bool save_orig = false;

	if (fprog->len == 0 || fprog->len > BPF_MAXINSNS)
		return ERR_PTR(-EINVAL);

	/* Avoid integer overflow in allocation size calculation */
	if (INT_MAX / fprog->len < sizeof(struct sock_filter))
		return ERR_PTR(-EINVAL);
			    ...
			    const bool recheck_after_trace)
{
	...
	case SECCOMP_RET_TRACE:
		...
		this_syscall = syscall_get_nr(current, regs);
		if (this_syscall < 0)
			goto skip;
