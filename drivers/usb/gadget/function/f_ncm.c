				    ...
				    struct sk_buff *skb)
{
	...
	if (skb) {
		...
		if (!ncm->skb_tx_data) {
			...
			ntb_ndp = (__le16 *)skb_put(ncm->skb_tx_ndp, opts->ndp_size);
		...
		/* Delay the timer. */
		hrtimer_start(&ncm->task_timer, ns_to_ktime(TX_TIMEOUT_NSECS),
			      HRTIMER_MODE_REL);

		/* Add the datagram position entries */
		ntb_ndp = (__le16 *)skb_put_zero(ncm->skb_tx_ndp, dgram_idx_len);
