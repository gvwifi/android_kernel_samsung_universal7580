...
{
	int ret;
	printk(KERN_ERR "usb_gadget_udc_start: udc=%p gadget=%p ops=%p udc_start=%p\n",
		udc, udc->gadget, udc->gadget->ops, udc->gadget->ops->udc_start);
	ret = udc->gadget->ops->udc_start(udc->gadget, udc->driver);
	printk(KERN_ERR "usb_gadget_udc_start: udc_start returned %d\n", ret);
	return ret;
}
...
static int udc_bind_to_driver(struct usb_udc *udc, struct usb_gadget_driver *driver)
{
	...
	pr_err("udc_bind_to_driver: calling driver->bind, driver=%p bind=%p\n",
		driver, driver->bind);
	ret = driver->bind(udc->gadget, driver);
	pr_err("udc_bind_to_driver: driver->bind returned %d\n", ret);
	if (ret)
		goto err1;
	pr_err("udc_bind_to_driver: calling usb_gadget_udc_start\n");
	ret = usb_gadget_udc_start(udc);
	pr_err("udc_bind_to_driver: usb_gadget_udc_start returned %d\n", ret);
	if (ret) {
