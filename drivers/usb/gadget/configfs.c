		...
		const char *page, size_t len)
{
	...
	if (!strlen(name) || strcmp(name, "none") == 0) {
		...
		if (gi->composite.gadget_driver.udc_name) {
			/* Already bound to a UDC - if same one, succeed silently */
			if (strcmp(gi->composite.gadget_driver.udc_name, name) == 0) {
				pr_info("USB_DBG: UDC_store: already bound to '%s', skipping rebind\n", name);
				kfree(name);
				mutex_unlock(&gi->lock);
				return len;
			}
		...
		pr_info("USB_DBG: UDC_store: gadget driver bound OK\n");
	}
	mutex_unlock(&gi->lock);
	return len;
err:
	...
	struct config_item *usb_func_ci)
{
	...
	pr_info("USB_DBG: config_usb_cfg_link: linking '%s' to config\n", usb_func_ci->ci_name);
	...
	if (IS_ERR(f)) {
		pr_err("USB_DBG: config_usb_cfg_link: usb_get_function FAILED: %ld\n", PTR_ERR(f));
		ret = PTR_ERR(f);
		goto out;
	}
	...
	pr_info("USB_DBG: config_usb_cfg_link: function linked to config OK\n");
	ret = 0;
out:
	mutex_unlock(&gi->lock);
	return ret;
		...
		const char *name)
{
	...
	*instance_name = '\0';
	...
	pr_info("USB_DBG: function_make: creating function '%s' instance '%s'\n", func_name, instance_name);
	fi = usb_get_function_instance(func_name);
	if (IS_ERR(fi)) {
		pr_err("USB_DBG: function_make: usb_get_function_instance('%s') FAILED: %ld\n", func_name, PTR_ERR(fi));
		return ERR_CAST(fi);
	}
	pr_info("USB_DBG: function_make: usb_get_function_instance('%s') OK\n", func_name);
	...
	mutex_unlock(&gi->lock);
	pr_info("USB_DBG: function_make: '%s' added to available_func list OK\n", name);
	return &fi->group;
}
		...
		struct usb_gadget_driver *gdriver)
{
	...
	list_for_each_entry(c, &gi->cdev.configs, list) {
		struct config_usb_cfg *cfg;
		struct usb_function *f_dbg;
		int func_count = 0;

		cfg = container_of(c, struct config_usb_cfg, c);
		list_for_each_entry(f_dbg, &cfg->func_list, list)
			func_count++;
		pr_info("USB_DBG: configfs_composite_bind: config '%s'/%d has %d functions\n",
			c->label, c->bConfigurationValue, func_count);
	...
	list_for_each_entry(c, &gi->cdev.configs, list) {
		...
		list_for_each_entry_safe(f, tmp, &cfg->func_list, list) {
			list_del(&f->list);
			pr_err("USB_DBG: configfs_composite_bind: calling usb_add_function for '%s'\n",
				f->name ? f->name : "(null)");
			ret = usb_add_function(c, f);
			pr_err("USB_DBG: configfs_composite_bind: usb_add_function returned %d\n", ret);
			if (ret) {
	...
	usb_ep_autoconfig_reset(cdev->gadget);
	pr_err("USB_DBG: configfs_composite_bind: SUCCESS, returning 0\n");
	return 0;

err_purge_funcs:
	pr_err("USB_DBG: configfs_composite_bind: err_purge_funcs, ret=%d\n", ret);
	purge_configs_funcs(gi);
err_comp_cleanup:
	pr_err("USB_DBG: configfs_composite_bind: err_comp_cleanup, ret=%d\n", ret);
