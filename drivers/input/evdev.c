...
struct evdev_client {
	...
	int clkid;
	unsigned long *evmasks[EV_CNT];	/* per-type event filtering masks */
	unsigned int bufsize;
	struct input_event buffer[];
};

/* ---- EVIOCSMASK: event filtering support (backported from 3.12+) ---- */

static size_t evdev_get_mask_cnt(unsigned int type)
{
	static const size_t counts[EV_CNT] = {
		/* EV_SYN==0 is EV_CNT, see EVIOCGBIT */
		[EV_SYN]	= EV_CNT,
		[EV_KEY]	= KEY_CNT,
		[EV_REL]	= REL_CNT,
		[EV_ABS]	= ABS_CNT,
