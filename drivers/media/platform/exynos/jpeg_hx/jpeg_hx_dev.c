...
static int jpeg_clk_get(struct jpeg_dev *jpeg)
{
	...
	char *gate_clkname, *gate2_clkname;
	...
	of_property_read_string_index(dev->of_node,
		"clock-names", JPEG_GATE2_CLK, (const char **)&gate2_clkname);
	...
	jpeg->clk2 = clk_get(dev, gate2_clkname);
	if (IS_ERR(jpeg->clk2))
		dev_warn(dev, "gate2 clk not provided\n");
...
static void jpeg_clock_gating(struct jpeg_dev *jpeg, enum jpeg_clk_status status)
{
	if (status == JPEG_CLK_ON) {
		...
		if (clk_cnt < 0) {
			...
			clk_unprepare(jpeg->clk);
			if (!IS_ERR(jpeg->clk2))
				clk_disable_unprepare(jpeg->clk2);
