...
struct sk_filter {
	atomic_t	refcnt;
	u32		len;	/* Number of filter blocks */
	bool		bpf_sock_ops_cb_flags; /* val to communicate with BPF_MAP_TYPE_SOCK_OPS */
	struct rcu_head	rcu;
...
#define SK_RUN_FILTER(filter, ctx) BPF_PROG_RUN((filter)->prog, ctx)
...
/* 3.10 sock.h provides inline implementations */
...
/* Internal BPF ancillary flag - must not overlap with BPF opcode bits */
