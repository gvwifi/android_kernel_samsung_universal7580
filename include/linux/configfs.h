...
/* Legacy macros for sdcardfs and other old drivers */
#define CONFIGFS_ATTR_STRUCT(_pfx)					\
struct _pfx##_attribute {						\
	struct configfs_attribute attr;					\
	ssize_t (*show)(struct _pfx *, char *);				\
	ssize_t (*store)(struct _pfx *, const char *, size_t);		\
}

#define __CONFIGFS_ATTR(_name, _mode, _show, _store)			\
{									\
	.attr	= { .ca_name = __stringify(_name), .ca_mode = _mode },	\
	.show	= _show,						\
	.store	= _store,						\
}
...
{									\
	.attr	= { .ca_name = __stringify(_name), .ca_mode = 0444 },	\
	.show	= _show,						\
}

#define CONFIGFS_ATTR_OPS(_pfx)						\
static ssize_t _pfx##_attr_show(struct config_item *item,		\
				struct configfs_attribute *attr,	\
				char *page)				\
{									\
	struct _pfx *p = to_##_pfx(item);				\
	struct _pfx##_attribute *a = container_of(attr, struct _pfx##_attribute, attr); \
	if (a->show)							\
		return a->show(p, page);				\
	return 0;							\
}									\
static ssize_t _pfx##_attr_store(struct config_item *item,		\
				 struct configfs_attribute *attr,	\
...
struct configfs_item_operations {
	...
	/* Legacy callbacks for old drivers like sdcardfs */
