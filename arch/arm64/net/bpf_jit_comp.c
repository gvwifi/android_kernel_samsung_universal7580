...
emit_cond_jmp:
		...
		case BPF_JSLE:
			...
			break;
		default:
			pr_err("bpf_jit: unknown opcode %02x (imm: %d, off: %d, src: %d, dst: %d)\n",
				code, imm, off, insn->src_reg, insn->dst_reg);
			return -EFAULT;
		}
...
#ifdef CONFIG_AS_LSE
		/* LSE atomics not available in kernel 3.10 - always use LDXR/STXR */
		if (0 && cpus_have_cap(ARM64_HAS_LSE_ATOMICS)) {
#else
		if (0) {
#endif
...
struct bpf_prog *bpf_int_jit_compile(struct bpf_prog *prog)
{
	...
	if (!bpf_jit_enable) {
		pr_info("bpf_jit: FAIL - bpf_jit_enable is 0\n");
		return orig_prog;
	}
	 ...
	 */
	if (IS_ERR(tmp)) {
		pr_info("bpf_jit: FAIL - bpf_jit_blind_constants returned error\n");
		return orig_prog;
	}
	if (tmp != prog) {
		tmp_blinded = true;
	...
	if (ctx.offset == NULL) {
		pr_info("bpf_jit: FAIL - kcalloc for ctx.offset failed\n");
		prog = orig_prog;
		goto out;
	}
	...
	if (build_body(&ctx)) {
		pr_info("bpf_jit: FAIL - build_body (first pass) failed\n");
		prog = orig_prog;
		goto out_off;
	}

	if (build_prologue(&ctx)) {
		pr_info("bpf_jit: FAIL - build_prologue failed\n");
		prog = orig_prog;
		goto out_off;
	}
	...
	if (header == NULL) {
		pr_info("bpf_jit: FAIL - bpf_jit_binary_alloc failed (image_size=%d)\n", image_size);
		prog = orig_prog;
		goto out_off;
