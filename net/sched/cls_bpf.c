 ...
 *
 * Simplified version for Linux 3.10 - adapted from 4.14
 *
...
/* Extension map for tc filter extensions */
...
{
	tcf_exts_destroy(prog->tp, &prog->exts);
...
static int cls_bpf_prog_from_efd(struct nlattr **tb, struct cls_bpf_prog *prog)
{
	...
	if (tb[TCA_BPF_NAME]) {
		int len = nla_len(tb[TCA_BPF_NAME]);
		name = kmalloc(len, GFP_KERNEL);
		if (!name) {
			bpf_prog_put(fp);
			return -ENOMEM;
		}
		nla_strlcpy(name, tb[TCA_BPF_NAME], len);
	}
				   ...
				   struct nlattr *est)
{
	...
	memset(&e, 0, sizeof(e));
	ret = tcf_exts_validate(net, tp, tb, est, &e, &bpf_ext_map);
	if (ret < 0)
		return ret;
	...
	tcf_exts_change(tp, &prog->exts, &e);
	return 0;
}
			  ...
			  unsigned long *arg)
{
	...
	memset(&prog->exts, 0, sizeof(prog->exts));
	prog->tp = tp;
	...
	if (handle == 0) {
		handle = cls_bpf_grab_new_handle(tp, head);
		if (handle == 0) {
			ret = -EINVAL;
			goto errout;
		}
