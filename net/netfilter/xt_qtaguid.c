...
{
	return uid_eq(make_kuid(&init_user_ns, uid), current_fsuid()) || can_manipulate_uids();
}
...
static bool can_read_other_uid_stats(uid_t uid)
{
	...
	return in_egroup_p(xt_qtaguid_stats_file->gid)
		|| unlikely(uid_eq(current_fsuid(), GLOBAL_ROOT_UID)) || uid_eq(make_kuid(&init_user_ns, uid), current_fsuid())
...
static int ctrl_cmd_delete(const char *input)
{
	...
	if (tcs_entry) {
		CT_DEBUG("qtaguid: ctrl_delete(%s): "
			 "erase tcs: tag=0x%llx (uid=%u) set=%u\n",
			 input,
			 tcs_entry->tn.tag,
...
static int ctrl_cmd_untag(const char *input)
{
	...
	if (IS_ERR_OR_NULL(pqd_entry))
		pr_warn_once("qtaguid: %s(): "
			     "User space forgot to untag on socket close. "
			     "stats=0x%llx ctrl.mq=0x%llx pid=%u tgid=%u "
			     "uid=%u\n", __func__,
			     (u64)sock_tag_entry->tag, (u64)sock_tag_entry->tag,
			     current->pid, current->tgid,
			     from_kuid(&init_user_ns, current_fsuid()));
	else
