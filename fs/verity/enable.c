				   ...
				   struct ahash_request *req)
{
	...
	for (i = 0; i < num_blocks_to_hash; i++) {
		...
		if (level == 0) {
			 ...
			 *
			 *
			 * For Merkle tree blocks, we can do readahead up to
			 * 8 blocks (32KB) or BIO_MAX_PAGES, whichever is
			 * smaller. Filesystems may further limit this limit.
			 */
			pages = min_t(unsigned int, params->block_size * 8 / PAGE_SIZE,
				      BIO_MAX_PAGES);
			num_ra_pages = min_t(unsigned long, num_blocks_to_hash - i, pages);
			 ...
			 const struct fsverity_enable_arg *arg)
{
	...
	/*
	 * On kernel 3.10, ext4_write_end() extends i_size when writing past
	 * EOF (for the Merkle tree metadata).  Restore i_size to the original
	 * data size so that fsverity_create_info() can validate that
	 * desc->data_size matches inode->i_size.  The filesystem-specific
	 * ->end_enable_verity() will do the final i_size/i_disksize fixup.
	 */
	{
		loff_t data_size = le64_to_cpu(desc->data_size);
		if (inode->i_size != data_size) {
			pr_debug("Restoring i_size from %lld to %lld after Merkle tree build\n",
