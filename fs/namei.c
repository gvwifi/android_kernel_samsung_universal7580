...
static int sb_permission(struct super_block *sb, struct inode *inode, int mask)
{
	if (unlikely(mask & MAY_WRITE)) {
		...
		if ((sb->s_flags & MS_RDONLY) &&
			...
			if (printk_timed_ratelimit(&last_print, 1000))
				pr_debug("VFS DEBUG: sb_permission returning EROFS due to MS_RDONLY on %s (ino=%lu)\n", sb->s_id, inode->i_ino);
			return -EROFS;
		}
	}
	return 0;
		...
		int, newdfd, const char __user *, newname, unsigned int, flags)
{
	...
	/* RENAME_WHITEOUT requires RENAME_EXCHANGE to be unset */
	if ((flags & RENAME_WHITEOUT) && (flags & RENAME_EXCHANGE))
		return -EINVAL;

	/* We don't support RENAME_EXCHANGE and RENAME_WHITEOUT in this kernel */
