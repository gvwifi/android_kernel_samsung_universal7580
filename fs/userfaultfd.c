		     ...
		     unsigned int flags, unsigned long reason)
{
	...
	VM_BUG_ON(reason & ~(VM_UFFD_MISSING|VM_UFFD_WP|VM_UFFD_MINOR));
	VM_BUG_ON(!(reason & VM_UFFD_MISSING) ^ !!(reason & VM_UFFD_WP) ^ !!(reason & VM_UFFD_MINOR));
...
static int userfaultfd_release(struct inode *inode, struct file *file)
{
	...
	for (vma = mm->mmap; vma; vma = vma->vm_next) {
		...
		new_flags = vma->vm_flags & ~(VM_UFFD_MISSING | VM_UFFD_WP | VM_UFFD_MINOR);
		if (still_valid) {
				...
				unsigned long arg)
{
	...
	if (uffdio_register.mode & UFFDIO_REGISTER_MODE_MINOR) {
		...
		/*
		 * Minor faults are only supported on shmem and hugetlbfs.
		 */
		// We will check for vma type in the loop
	}
	...
	for (cur = vma; cur && cur->vm_start < end; cur = cur->vm_next) {
		...
		BUG_ON(!!cur->vm_userfaultfd_ctx.ctx ^
		       !!(cur->vm_flags & (VM_UFFD_MISSING | VM_UFFD_WP | VM_UFFD_MINOR)));
...
static struct file *userfaultfd_file_create(int flags)
{
	...
	/* Strip UFFD_USER_MODE_ONLY - accepted but not stored as file flag */
