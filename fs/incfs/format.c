...
static int append_zeros(struct backing_file_context *bfc, size_t len)
{
	...
	return incfs_vfs_fallocate_compat(bfc->bc_file, 0, new_last_byte_offset, 1);
}
...
{
	return incfs_kernel_read_compat(f, buf, size, &pos);
}

ssize_t incfs_kwrite(struct file *f, const void *buf, size_t size, loff_t pos)
{
	return incfs_kernel_write_compat(f, buf, size, &pos);
}
