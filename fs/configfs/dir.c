...
static int configfs_readdir(struct file *file, void *dirent, filldir_t filldir)
{
	struct dentry *dentry = file->f_path.dentry;
	...
	if (file->f_pos == 0) {
		if (filldir(dirent, ".", 1, 0, dentry->d_inode->i_ino, DT_DIR) < 0)
			return 0;
		file->f_pos++;
	}
	if (file->f_pos == 1) {
	...
	for (p = q->next; p != &parent_sd->s_children; p = p->next) {
		...
		sd_dentry = next->s_dentry;
		if (sd_dentry)
			inode = d_inode(sd_dentry);
		if (inode)
			ino = inode->i_ino;
		spin_unlock(&configfs_dirent_lock);
		if (!inode)
		...
		if (filldir(dirent, name, len, file->f_pos, ino, dt_type(next)) < 0)
			return 0;
